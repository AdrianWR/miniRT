...
/*   Updated: 2020/09/01 23:10:07 by aroque           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_world		*new_world(t_window window)
{
	...
	world->ambient = new_light(point(0,0,0), 0.3, 0x00FFFFFF);
	return (world);
}
...
t_list	*new_light_set(void)
{
	...
	//ft_lstadd_back(&light_set, ft_lstnew(
	//			new_light(point(0, 6, -4), 0.2, 0x0000FF00)));
	return (light_set);
}
...
t_list	*new_camera_set(t_window window)
{
	...
	ft_lstadd_back(&camera_set, ft_lstnew(
		new_cam(point(0, 30, 30), vector(0, -0.5, -0.82), 60, window)));
	ft_lstadd_back(&camera_set, ft_lstnew(
